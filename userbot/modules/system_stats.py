...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit(
            "**⚜-**⚡Yanzzx-Userbot⚡ Versi:** \n "
            f"heads/Yanzzx-Userbot-0-x634i7u1"
            "\n**⚜-**Revisi:**\n "
            f"{revout}"
        )
    else:
...
@register(outgoing=True, pattern=r"^\.(?:Yanzzxalive)\s?(.)?")
...
@register(outgoing=True, pattern=r"^\.(?:Yanzzxon)\s?(.)?")
async def amireallyalive(alive):
    await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
...
CMD_HELP.update(
    {
        "alive": "𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.alive` or `.on` or `Yanzzx`"
